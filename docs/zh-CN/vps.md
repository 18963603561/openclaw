...
title: Linux 服务器
x-i18n:
    generated_at: "2026-04-09T01:03:21Z"
    model: gpt-5.4
    provider: openai
...
你可以在任意 Linux 服务器或云 VPS 上运行 OpenClaw Gateway 网关。本页将帮助你
选择提供商，说明云部署的工作方式，并介绍适用于各类环境的通用 Linux
调优方法。

## 选择提供商

<CardGroup cols={2}>
  <Card title="Railway" href="/install/railway">一键安装，浏览器配置</Card>
  <Card title="Northflank" href="/install/northflank">一键安装，浏览器配置</Card>
...
如果所有用户都处于同一信任边界内，并且该智能体仅用于业务用途，那么为团队运行单个智能体是可行的方案。

- 请将其运行在专用运行时上（VPS/VM/容器 + 专用 OS 用户/账户）。
- 不要让该运行时登录个人 Apple/Google 账户，或个人浏览器/密码管理器配置文件。
- 如果用户之间彼此存在对抗关系，请按 gateway/主机/OS 用户拆分。

安全模型详情请参见：[安全](/zh-CN/gateway/security)。

## 在 VPS 中使用节点

你可以将 Gateway 网关保留在云端，并在你的本地设备上配对**节点**
...
- 首次运行命令会预热缓存；后续运行会更快。
- 关于 Raspberry Pi 的具体说明，请参见 [Raspberry Pi](/zh-CN/install/raspberry-pi)。
...
- 保持重启行为显式：
  ...
  - `RestartSec=2`
  - `TimeoutStartSec=90`
- 状态/缓存路径优先使用 SSD 支撑的磁盘，以减少随机 I/O 冷启动损耗。
...
如果你是有意安装了系统单元，请改为通过 `sudo systemctl edit openclaw-gateway.service`
编辑 `openclaw-gateway.service`。
