...
title: 入门指南
x-i18n:
    generated_at: "2026-04-08T07:10:55Z"
    model: gpt-5.4
    provider: openai
...
安装 OpenClaw、运行新手引导，并与你的 AI 助手聊天——整个过程大约只需 5 分钟。完成后，你将拥有一个正在运行的 Gateway 网关、已配置的认证，以及一个可用的聊天会话。

## 你需要准备什么

- **Node.js** —— 推荐 Node 24（也支持 Node 22.14+）
- **一个模型 provider 的 API key**（Anthropic、OpenAI、Google 等）——新手引导会提示你输入

<Tip>
使用 `node --version` 检查你的 Node 版本。
**Windows 用户：** 原生 Windows 和 WSL2 都受支持。WSL2 更稳定，且推荐用于完整体验。见 [Windows](/zh-CN/platforms/windows)。
需要安装 Node？见 [Node 设置](/zh-CN/install/node)。
</Tip>
...
/>
  ...
  <Step title="运行新手引导">
    ...
    向导会引导你选择模型 provider、设置 API key
    以及配置 Gateway 网关。整个过程大约需要 2 分钟。
  ...
  <Step title="验证 Gateway 网关正在运行">
    ...
    你应该能看到 Gateway 网关正在监听 18789 端口。
  ...
  <Step title="打开仪表板">
    ...
    这会在浏览器中打开 Control UI。如果它能正常加载，就说明一切工作正常。

  </Step>
  <Step title="发送你的第一条消息">
    在 Control UI 聊天界面中输入一条消息，你应该会收到 AI 回复。
...
<Accordion title="高级：挂载自定义 Control UI 构建">
  如果你维护了一个本地化或自定义的仪表板构建，请将
  `gateway.controlUi.root` 指向一个包含已构建静态资源
  和 `index.html` 的目录。
