...
title: 展示
x-i18n:
    generated_at: "2026-04-08T07:14:11Z"
    model: gpt-5.4
    provider: openai
...
<Info>
**想被展示吗？** 请在 [Discord 的 #self-promotion](https://discord.gg/clawd) 中分享你的项目，或在 [X 上 @openclaw](https://x.com/openclaw)。
</Info>

## 🎥 运行中的 OpenClaw

VelvetShark 制作的完整设置演示（28 分钟）。
...
OpenCode 完成改动 → 打开一个 PR → OpenClaw 审查 Diffs，并在 Telegram 中回复“有一些小建议”，同时给出明确的合并结论（包括需要先修复的关键问题）。
...
让 “Robby”（@openclaw）为本地酒窖构建一个 Skill。它会请求一个 CSV 导出示例以及存储位置，然后快速构建 / 测试该 Skill（示例中有 962 瓶酒）。
...
每周膳食计划 → 常购商品 → 预订配送时间段 → 确认订单。无需 API，只用浏览器控制。
...
通过热键选中屏幕区域 → Gemini 视觉 → 即时将 Markdown 放入你的剪贴板。
...
用于在 Agents、Claude、Codex 和 OpenClaw 之间管理 Skills / 命令的桌面应用。

  <img src="/assets/showcase/agents-ui.jpg" alt="Agents UI app" />
</Card>

<Card title="通过 Telegram 发送语音便笺（papla.media）" icon="microphone" href="https://papla.media/docs">
...
通过 Homebrew 安装的 helper，用于列出 / 检查 / 监听本地 OpenAI Codex 会话（CLI + VS Code）。
...
一个 Gateway 网关下管理 14+ 智能体，由 Opus 4.5 编排器委派给 Codex worker。附有完整的[技术说明](https://github.com/adam91holt/orchestrated-ai-articles)，涵盖 Dream Team 成员、模型选择、沙箱隔离、Webhooks、heartbeats 和委派流程。用于智能体沙箱隔离的 [Clawdspace](https://github.com/adam91holt/clawdspace)。[博客文章](https://adams-ai-journey.ghost.io/2026-the-year-of-the-orchestrator/)。
</Card>

<Card title="Linear CLI" icon="terminal" href="https://github.com/Finesssee/linear-cli">
  **@NessZerra** • `devtools` `linear` `cli` `issues`

一个适用于 Linear 的 CLI，可与智能体工作流（Claude Code、OpenClaw）集成。在终端中管理 issue、项目和工作流。首个外部 PR 已合并！
</Card>

<Card title="Beeper CLI" icon="message" href="https://github.com/blqke/beepcli">
  **@jules** • `messaging` `beeper` `cli` `automation`

通过 Beeper Desktop 读取、发送和归档消息。使用 Beeper 本地 MCP API，使智能体可以在一个地方管理你的所有聊天（iMessage、WhatsApp 等）。
</Card>
...
Claude Code 发现并确认了净化器控制方式，然后由 OpenClaw 接管房间空气质量管理。
...
由屋顶摄像头触发：每当天空很美时，就让 OpenClaw 拍一张照片——它设计了一个 Skill 并完成了拍摄。
...
通过定时提示词，每天早上生成一张“场景”图片（天气、任务、日期、最喜欢的帖子 / 引语），由一个 OpenClaw 角色完成。
</Card>

<Card title="Padel 球场预订" icon="calendar-check" href="https://github.com/joshp123/padel-cli">
  **@joshp123** • `automation` `booking` `cli`
  
  Playtomic 空位检查 + 预订 CLI。再也不会错过空球场。
  
  <img src="/assets/showcase/padel-screenshot.jpg" alt="padel-cli screenshot" />
</Card>

<Card title="会计资料采集" icon="file-invoice-dollar">
  **社区** • `automation` `email` `pdf`
  
  从邮件中收集 PDF，并为税务顾问整理文档。每月会计工作进入自动驾驶。
</Card>

<Card title="沙发土豆开发模式" icon="couch" href="https://davekiss.com">
  **@davekiss** • `telegram` `website` `migration` `astro`

一边看 Netflix，一边通过 Telegram 重建整个个人网站——Notion → Astro，迁移了 18 篇文章，DNS 切到 Cloudflare。全程没打开笔记本电脑。
</Card>
...
OpenClaw 连接到 Jira 后，动态生成了一个新的 Skill（那时它还不存在于 ClawHub 上）。
</Card>
...
监听公司 Slack 渠道，提供有帮助的回复，并将通知转发到 Telegram。它曾在无人要求的情况下，自主修复了一个已部署应用中的生产 bug。
</Card>
...
<Card title="WhatsApp 记忆库" icon="vault">
  **社区** • `memory` `transcription` `indexing`
  ...
  导入完整的 WhatsApp 导出数据，转录 1000+ 条语音便笺，与 git 日志交叉核对，并输出带链接的 markdown 报告。
</Card>
...
<Card title="《头脑特工队 2》记忆模型" icon="brain">
...
<Card title="GoHome 自动化" icon="house-signal" href="https://github.com/joshp123/gohome">
  **@joshp123** • `home` `nix` `grafana`
  ...
  基于 Nix 的原生家庭自动化，以 OpenClaw 作为交互界面，并配有精美的 Grafana 仪表板。
...
<Card title="StarSwap Marketplace" icon="star" href="https://star-swap.com/">
  **社区** • `marketplace` `astronomy` `webapp`
  ...
  一个完整的天文设备交易市场。基于 / 围绕 OpenClaw 生态系统构建。
</Card>
...
<Steps>
  ...
  <Step title="附上细节">
    告诉我们它的作用，附上仓库 / 演示链接，如果有截图也请一并分享
  </Step>
  <Step title="获得展示">
    我们会将其中突出的项目添加到本页
  </Step>
