...
read_when:
    - 你正在脚本或 CI 中自动化新手引导
    - 你需要特定 provider 的非交互式示例
sidebarTitle: CLI automation
summary: 用于 OpenClaw CLI 的脚本化新手引导与智能体设置
title: CLI 自动化
x-i18n:
    generated_at: "2026-04-08T07:14:33Z"
    model: gpt-5.4
    provider: openai
...
使用 `--secret-input-mode ref` 可在 auth profiles 中存储基于环境变量的引用，而不是明文值。
在新手引导流程中，也可以交互式选择使用环境变量引用或已配置的 provider 引用（`file` 或 `exec`）。

在非交互 `ref` 模式下，provider 环境变量必须在进程环境中设置。
如果传入内联 key 标志，但没有对应的环境变量，现在会快速失败。
...
<AccordionGroup>
  ...
  <Accordion title="OpenCode 示例">
    ...
    openclaw onboard --non-interactive \
      ...
      --gateway-bind loopback
    ```
    若要使用 Go 目录，请改为 `--auth-choice opencode-go --opencode-go-api-key "$OPENCODE_API_KEY"`。
  </Accordion>
...
Anthropic setup-token 仍作为受支持的新手引导 token 路径保留，但当 Claude CLI 复用可用时，OpenClaw 现在更倾向于使用它。
生产环境中请优先使用 Anthropic API key。

## 添加另一个智能体

使用 `openclaw agents add <name>` 创建一个独立智能体，它拥有自己的工作区、
会话和 auth profiles。不带 `--workspace` 运行时会启动向导。
