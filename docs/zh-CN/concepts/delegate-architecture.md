...
这将 [Multi-Agent Routing](/zh-CN/concepts/multi-agent) 从个人使用扩展到组织级部署。
...
- 遵循**[standing orders](/zh-CN/automation/standing-orders)**——这些规则定义在智能体的 `AGENTS.md` 中，用于指定它哪些行为可以自主执行，哪些需要人工批准（定时执行请参见 [Cron Jobs](/zh-CN/automation/cron-jobs)）。
...
这一层级结合了 Tier 2 权限与 [Cron Jobs](/zh-CN/automation/cron-jobs) 和 [Standing Orders](/zh-CN/automation/standing-orders)。
...
参见 [沙箱隔离](/gateway/sandboxing) 和 [Multi-Agent Sandbox & Tools](/tools/multi-agent-sandbox-tools)。
...
使用 [Multi-Agent Routing](/zh-CN/concepts/multi-agent) 绑定，将入站消息路由到 delegate 智能体：
...
绝不要将主智能体的 `agentDir` 与 delegate 共享。关于认证隔离的详细信息，请参见 [Multi-Agent Routing](/zh-CN/concepts/multi-agent)。
...
delegate 的 `AGENTS.md` 定义了它的自主权限范围——哪些事情它可以不经询问直接执行，哪些需要批准，哪些被禁止。[Cron Jobs](/zh-CN/automation/cron-jobs) 驱动其日常计划。
...
4. **定义 [standing orders](/zh-CN/automation/standing-orders)** 以支持自主运行。
