...
read_when:
    - 你想了解默认记忆后端
    - 你想配置嵌入提供商或混合搜索
summary: 基于默认 SQLite 的记忆后端，支持关键词、向量和混合搜索
title: 内置记忆引擎
x-i18n:
    generated_at: "2026-04-08T04:00:31Z"
    model: gpt-5.4
    provider: openai
...
内置引擎是默认的记忆后端。它将你的记忆索引存储在每个智能体独立的 SQLite 数据库中，且入门时无需任何额外依赖。
...
| Voyage | `voyage`  | 是       |                                     |
| Mistral | `mistral` | 是      |                                     |
| Ollama | `ollama`  | 否       | 本地使用，需显式设置                |
| Local  | `local`   | 是（优先） | GGUF 模型，下载大小约 0.6 GB      |

自动检测会按照表中所示顺序，选择第一个能够解析出 API 密钥的提供商。可设置 `memorySearch.provider` 进行覆盖。

## 索引的工作方式

OpenClaw 会将 `MEMORY.md` 和 `memory/*.md` 索引为多个分块（约 400 个 token，含 80 个 token 重叠），并将它们存储在每个智能体独立的 SQLite 数据库中。

- **索引位置：** `~/.openclaw/memory/<agentId>.sqlite`
- **文件监听：** 记忆文件发生更改时，会触发带防抖的重新索引（1.5 秒）。
- **自动重新索引：** 当嵌入提供商、模型或分块配置发生变化时，会自动重建整个索引。
...
如果你需要重排序、查询扩展，或想为工作区外的目录建立索引，可考虑切换到 [QMD 记忆引擎](/zh-CN/concepts/memory-qmd)。

如果你想要支持跨会话记忆并自动进行用户建模，可考虑 [Honcho 记忆](/zh-CN/concepts/memory-honcho)。

## 故障排除

**记忆搜索已禁用？** 检查 `openclaw memory status`。如果未检测到提供商，请显式设置一个，或添加 API 密钥。

**结果过旧？** 运行 `openclaw memory index --force` 进行重建。监听器在极少数边界情况下可能会漏掉变更。

**sqlite-vec 无法加载？** OpenClaw 会自动回退到进程内余弦相似度计算。请查看日志以获取具体加载错误。

## 配置

关于嵌入提供商设置、混合搜索调优（权重、MMR、时间衰减）、批量索引、多模态记忆、sqlite-vec、额外路径以及所有其他配置项，请参见
