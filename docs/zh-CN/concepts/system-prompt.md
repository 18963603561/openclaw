...
如果你想让智能体听起来不那么通用，可以先从 [SOUL.md ????](/zh-CN/concepts/soul) 开始。

如果你想检查每个注入文件贡献了多少内容（原始值与注入值、截断情况，以及工具 schema 开销），请使用 `/context list` 或 `/context detail`。参见 [???](/zh-CN/concepts/context)。
