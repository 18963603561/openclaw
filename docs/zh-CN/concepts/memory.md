...
在 [??](/zh-CN/concepts/compaction) 对你的对话进行总结之前，OpenClaw
...
- [内置记忆引擎](/zh-CN/concepts/memory-builtin) —— 默认的 SQLite 后端
- [QMD 记忆引擎](/zh-CN/concepts/memory-qmd) —— 高级本地优先 sidecar
- [Honcho 记忆](/zh-CN/concepts/memory-honcho) —— AI 原生跨会话 memory
- [记忆搜索](/zh-CN/concepts/memory-search) —— 搜索流水线、提供商与调优
- [Dreaming（实验性）](/zh-CN/concepts/dreaming) —— 将短期回溯内容在后台提升到长期记忆
- [Memory ????](/zh-CN/reference/memory-config) —— 所有配置项
- [??](/zh-CN/concepts/compaction) —— compaction 如何与 memory 交互
