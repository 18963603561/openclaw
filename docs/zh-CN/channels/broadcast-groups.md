...
read_when:
    - 配置广播组时
    - 调试 WhatsApp 中的多智能体回复时
status: experimental
summary: 将一条 WhatsApp 消息广播给多个智能体
title: 广播组
x-i18n:
    generated_at: "2026-04-08T03:40:35Z"
    model: gpt-5.4
    provider: openai
...
广播组允许多个智能体同时处理并回复同一条消息。这样你就可以创建由专业化智能体组成的团队，让它们在同一个 WhatsApp 群组或私信中协同工作 —— 全部使用同一个电话号码。

当前范围：**仅限 WhatsApp**（web 渠道）。

广播组会在渠道允许列表和群组激活规则之后进行评估。在 WhatsApp 群组中，这意味着广播会在 OpenClaw 正常应答时发生（例如：根据你的群组设置，在被提及时）。
...
注意：广播组不会绕过渠道允许列表或群组激活规则（提及/命令等）。它们只会改变消息在符合处理条件时 _由哪些智能体运行_。
...
1. **智能体数量上限：** 没有硬性上限，但超过 10 个智能体可能会变慢
