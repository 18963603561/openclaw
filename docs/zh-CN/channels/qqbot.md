...
read_when:
    ...
    - 你需要设置 QQ Bot 凭证
    - 你想使用 QQ Bot 群聊或私聊支持
summary: QQ Bot 设置、配置与用法
title: QQ Bot
x-i18n:
    generated_at: "2026-04-08T03:45:58Z"
    model: gpt-5.4
    provider: openai
...
插件支持 C2C 私聊、群 @ 消息，以及带富媒体（图片、语音、视频、文件）的 guild 频道消息。

状态：内置插件。支持私信、群聊、guild 频道和
媒体。不支持表情回应和线程。

## 内置插件

当前 OpenClaw 版本已内置 QQ Bot，因此常规打包构建不需要
单独执行 `openclaw plugins install` 步骤。

## 设置

1. 前往 [QQ 开放平台](https://q.qq.com/)，使用你的
   手机 QQ 扫码进行注册/登录。
2. 点击 **Create Bot** 创建一个新的 QQ 机器人。
3. 在机器人的设置页面中找到 **AppID** 和 **AppSecret** 并复制它们。

> AppSecret 不会以明文形式保存 —— 如果你离开页面前没有保存它，
> 你将不得不重新生成一个新的。
...
| `qqbot:channel:CHANNEL_ID` | guild 频道 |

> 每个机器人都有自己的一组用户 OpenID。通过机器人 A 接收到的 OpenID **不能**
> 用于通过机器人 B 发送消息。
...
| `/bot-help` | 列出所有命令 |
| `/bot-upgrade` | 显示 QQ Bot 升级指南链接 |
| `/bot-logs` | 将最近的 Gateway 网关 日志导出为文件 |

在任意命令后追加 `?` 可查看用法帮助（例如 `/bot-upgrade ?`）。

## 故障排除

- **机器人回复 “gone to Mars”：** 未配置凭证，或 Gateway 网关 未启动。
- **没有入站消息：** 请确认 `appId` 和 `clientSecret` 正确，并且
  该机器人已在 QQ 开放平台中启用。
- **使用 `--token-file` 设置后仍显示未配置：** `--token-file` 只会设置
  AppSecret。你仍然需要在配置中设置 `appId`，或提供 `QQBOT_APP_ID`。
- **主动消息未送达：** 如果用户最近没有互动，
  QQ 可能会拦截机器人主动发起的消息。
