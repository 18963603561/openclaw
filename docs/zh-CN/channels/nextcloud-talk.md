...
x-i18n:
    generated_at: "2026-04-08T03:44:14Z"
    model: gpt-5.4
    provider: openai
...
状态：内置插件（webhook bot）。支持私信、房间、反应和 Markdown 消息。

## 内置插件

Nextcloud Talk 在当前 OpenClaw 版本中作为内置插件提供，因此常规打包版本不需要单独安装。

如果你使用的是较旧版本，或排除了 Nextcloud Talk 的自定义安装，请手动安装：
...
1. 确保 Nextcloud Talk 插件可用。
   - 当前打包版 OpenClaw 已经内置该插件。
   - 较旧版本/自定义安装可使用上述命令手动添加。
2. 在你的 Nextcloud 服务器上，创建一个 bot：
...
- 默认值：`channels.nextcloud-talk.groupPolicy = "allowlist"`（受 mention 门控）。
- 使用 `channels.nextcloud-talk.rooms` 对房间加入 allowlist：
...
- 如果不允许任何房间，请保持 allowlist 为空，或设置 `channels.nextcloud-talk.groupPolicy="disabled"`。
...
- `channels.nextcloud-talk.rooms`：按房间配置的设置和 allowlist。
...
- `channels.nextcloud-talk.chunkMode`：`length`（默认）或 `newline`，后者会先按空行（段落边界）拆分，再按长度分块。
