...
title: 配对
x-i18n:
    generated_at: "2026-04-08T03:45:26Z"
    model: gpt-5.4
    provider: openai
...
- **1 小时后过期**。只有在创建了新的请求时，bot 才会发送配对消息（大致为每个发送者每小时一次）。
- 待处理的私信配对请求默认每个渠道上限为 **3** 个；在某个请求过期或被批准之前，其余请求会被忽略。
...
- 非默认账户只会读取/写入其作用域下的 allowlist 文件。
- 默认账户使用该渠道范围内的非作用域 allowlist 文件。

请将这些文件视为敏感信息（它们控制你助手的访问权限）。

重要说明：此存储仅用于私信访问。群组授权是独立的。
批准私信配对码并不会自动允许该发送者执行群组命令，也不会自动允许其在群组中控制 bot。对于群组访问，请配置该渠道的显式群组 allowlist（例如 `groupAllowFrom`、`groups`，或者按渠道不同使用按群组/按话题的覆盖项）。

## 2）节点设备配对（iOS/Android/macOS/无头节点）

节点作为**设备**连接到 Gateway 网关，并具有 `role: node`。Gateway 网关会创建一个设备配对请求，必须经过批准。
...
2. bot 会回复两条消息：一条说明消息，以及一条单独的**设置码**消息（方便在 Telegram 中复制粘贴）。
3. 在你的手机上，打开 OpenClaw iOS 应用 → Settings → Gateway。
4. 粘贴设置码并连接。
5. 回到 Telegram：`/pair pending`（查看请求 ID、角色和作用域），然后批准。

该设置码是一个经过 base64 编码的 JSON 负载，其中包含：

- `url`：Gateway 网关 WebSocket URL（`ws://...` 或 `wss://...`）
- `bootstrapToken`：一个短期、单设备使用的引导令牌，用于初始配对握手

该引导令牌携带内置的配对引导配置：

- 主交接的 `node` 令牌保持 `scopes: []`
- 任意交接的 `operator` 令牌仍然受限于引导 allowlist：
  `operator.approvals`、`operator.read`、`operator.talk.secrets`、`operator.write`
- 引导作用域检查按角色前缀执行，而不是一个扁平的统一作用域池：
  operator 作用域条目仅满足 operator 请求，而非 operator 角色仍必须在其自身角色前缀下请求作用域
...
如果同一设备使用不同的认证信息重试（例如不同的角色/作用域/公钥），先前的待处理请求会被替换，并创建新的 `requestId`。
...
- 旧版 `node.pair.*` API（CLI：`openclaw nodes pending|approve|reject|rename`）是一个独立的、由 Gateway 网关持有的配对存储。WS 节点仍然需要设备配对。
- 配对记录是已批准角色的持久事实来源。活动设备令牌仍然受限于该已批准角色集合；即使某个游离令牌条目位于批准角色之外，也不会产生新的访问权限。
