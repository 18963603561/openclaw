...
title: Twitch
x-i18n:
    generated_at: "2026-04-08T03:48:36Z"
    model: gpt-5.4
    provider: openai
...
通过 IRC 连接支持 Twitch 聊天。OpenClaw 以 Twitch 用户（机器人账户）身份连接，以便在频道中接收和发送消息。

## 内置插件

Twitch 已作为内置插件包含在当前 OpenClaw 版本中，因此普通打包构建无需单独安装。

如果你使用的是较旧版本，或未包含 Twitch 的自定义安装，请手动安装：
...
2. 为机器人创建一个专用 Twitch 账户（或使用现有账户）。
...
5. 配置 token：
   ...
   - 如果两者都设置了，以配置为准（环境变量回退仅适用于默认账户）。
6. 启动 Gateway 网关。

**⚠️ 重要：** 请添加访问控制（`allowFrom` 或 `allowedRoles`），以防止未授权用户触发机器人。`requireMention` 默认为 `true`。
...
优先使用 `allowFrom` 作为硬性 allowlist。如果你想使用基于角色的访问控制，请改用 `allowedRoles`。

**可用角色：** `"moderator"`、`"owner"`、`"vip"`、`"subscriber"`、`"all"`。

**为什么使用用户 ID？** 用户名可以更改，存在冒充风险。用户 ID 是永久不变的。
...
来自 [Twitch Token Generator](https://twitchtokengenerator.com/) 的 token 无法自动刷新——过期后请重新生成。

若要启用自动 token 刷新，请在 [Twitch Developer Console](https://dev.twitch.tv/console) 创建你自己的 Twitch 应用，并添加到配置中：
...
使用 `channels.twitch.accounts` 配置每个账户各自的 token。共享模式请参阅 [`gateway/configuration`](/zh-CN/gateway/configuration)。
...
**注意：** 每个账户都需要自己的 token（每个频道一个 token）。
...
`allowFrom` 是硬性 allowlist。设置后，仅这些用户 ID 被允许。
如果你想使用基于角色的访问控制，请不要设置 `allowFrom`，而是配置 `allowedRoles`：
...
**检查访问控制：** 确保你的用户 ID 在 `allowFrom` 中，或临时移除
...
- 检查 token 具有 `chat:read` 和 `chat:write` 作用域
- 如果使用 token 刷新，请确认已设置 `clientSecret` 和 `refreshToken`
...
- `username` - 机器人用户名
- `accessToken` - 具有 `chat:read` 和 `chat:write` 的 OAuth access token
...
- `channels.twitch.accounts.<accountName>` - 多账户配置（使用上述所有账户字段）
...
- **像对待密码一样对待 token** - 不要将 token 提交到 git
- **为长期运行的机器人启用自动 token 刷新**
- **使用用户 ID allowlists** 而不是用户名进行访问控制
- **监控日志** 以查看 token 刷新事件和连接状态
- **最小化 token 权限范围** - 仅请求 `chat:read` 和 `chat:write`
