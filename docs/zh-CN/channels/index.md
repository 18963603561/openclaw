...
- [Voice Call 插件](/zh-CN/plugins/voice-call) — 通过 Plivo 或 Twilio 实现电话通信（插件，需单独安装）。
...
- 出于安全考虑，会强制执行私信配对和 allowlists；请参阅 [安全](/zh-CN/gateway/security)。
- 故障排除：请参阅 [渠道故障排除](/zh-CN/channels/troubleshooting)。
- 模型提供商文档单独提供；请参阅 [模型提供商](/zh-CN/providers/models)。
