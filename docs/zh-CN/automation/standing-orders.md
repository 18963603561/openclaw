---
read_when:
    - 设置无需按任务逐次提示即可运行的 autonomous agent 工作流时
    - 定义智能体可以独立执行什么，以及哪些内容需要人工批准时
    - 为多程序智能体构建清晰的边界和升级规则时
summary: 为 autonomous agent 程序定义永久运行授权
title: 长期指令
x-i18n:
    generated_at: "2026-04-08T03:39:24Z"
    model: gpt-5.4
    provider: openai
...
长期指令会为你的智能体授予已定义程序的**永久运行授权**。你不必每次都提供单独的任务说明，而是定义具有明确范围、触发条件和升级规则的程序 —— 智能体会在这些边界内自主执行。

这就像每周五都对你的助手说“发送每周报告”，与授予长期授权之间的区别：“每周报告由你负责。每周五完成汇总并发送，只有在发现异常时才升级处理。”
...
长期指令定义在你的 [智能体工作区](/zh-CN/concepts/agent-workspace) 文件中。推荐做法是将它们直接写入 `AGENTS.md`（每个会话都会自动注入），这样智能体始终能在上下文中获取这些内容。对于更大的配置，你也可以将它们放在专用文件中，例如 `standing-orders.md`，并从 `AGENTS.md` 中引用它。
...
长期指令在与严格的执行纪律结合时效果最佳。长期指令中的每个任务都应遵循这个循环：

1. **执行** —— 完成实际工作（不要只是确认收到指令）
2. **验证** —— 确认结果正确（文件存在、消息已发送、数据已解析）
3. **报告** —— 告诉所有者已完成什么以及验证了什么
...
这种模式可以避免最常见的智能体失效方式：确认了任务，但没有真正完成。

## 多程序架构

对于管理多个事务的智能体，请将长期指令组织为边界清晰的独立程序：
...
- 从较窄的授权范围开始，随着信任建立逐步扩大
- 为高风险操作定义明确的审批关卡
- 包含“不要做什么”部分 —— 边界与权限同样重要
- 与 cron jobs 结合，实现可靠的基于时间的执行
- 每周查看智能体日志，验证长期指令是否得到遵守
- 随着你的需求演进更新长期指令 —— 它们是持续演进的文档

### 避免这样做

- 第一天就授予宽泛权限（“做任何你认为最好的事情”）
- 跳过升级规则 —— 每个程序都需要“何时停止并提问”的条款
- 假设智能体会记住口头指示 —— 把所有内容都写进文件
- 在单个程序中混合多个关注点 —— 不同领域应拆分为不同程序
- 忘记使用 cron jobs 落实执行 —— 没有触发条件的长期指令只会沦为建议
