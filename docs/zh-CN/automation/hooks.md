...
title: Hooks
x-i18n:
    generated_at: "2026-04-08T03:39:18Z"
    model: gpt-5.4
    provider: openai
...
Hooks 是一些在 Gateway 网关 内部发生特定事件时运行的小脚本。系统会自动从目录中发现它们，并且你可以通过 `openclaw hooks` 查看它们。

OpenClaw 中有两类 hooks：

- **内部 hooks**（本页）：当 Gateway 网关 内部触发智能体事件时运行，例如 `/new`、`/reset`、`/stop` 或生命周期事件。
- **Webhooks**：外部 HTTP 端点，允许其他系统在 OpenClaw 中触发工作。参见 [Webhooks](/zh-CN/automation/cron-jobs#webhooks)。
...
| `session:compact:before` | 压缩开始总结历史记录之前 |
...
**会话 patch 事件**（`session:patch`）：`context.sessionEntry`、`context.patch`（仅包含变更字段）、`context.cfg`。只有特权客户端才能触发 patch 事件。
...
系统会从以下目录中发现 hooks，覆盖优先级按从低到高排序：

1. **内置 hooks**：随 OpenClaw 一起发布
2. **插件 hooks**：内置在已安装插件中的 hooks
3. **托管 hooks**：`~/.openclaw/hooks/`（用户安装、在各工作区之间共享）。来自 `hooks.internal.load.extraDirs` 的额外目录与此优先级相同。
...
Hook 包是通过 `package.json` 中的 `openclaw.hooks` 导出 hooks 的 npm 包。安装方式如下：

```bash
openclaw plugins install <path-or-spec>
```

npm 规格仅支持 registry 形式（包名 + 可选精确版本或 dist-tag）。Git/URL/file 规格和 semver 范围会被拒绝。
...
| boot-md | `gateway:startup` | 在 Gateway 网关 启动时运行 `BOOT.md` |
...
<Note>
旧版的 `hooks.internal.handlers` 数组配置格式仍然支持，以保持向后兼容，但新的 hooks 应使用基于发现机制的系统。
</Note>
...
- **保持处理器快速执行。** Hooks 会在命令处理期间运行。对于较重的任务，请使用 `void processInBackground(event)` 以即发即忘方式处理。
- **妥善处理错误。** 用 try/catch 包裹高风险操作；不要抛出异常，以便其他处理器仍可运行。
...
2. 重启你的 Gateway 网关 进程，以便重新加载 hooks。
3. 检查 Gateway 网关 日志：`./scripts/clawlog.sh | grep hook`
