...
read_when:
    - 实现 macOS Canvas 面板时
    - 为可视化工作区添加智能体控制时
    - 调试 `WKWebView` canvas 加载问题时
summary: 通过 `WKWebView` + 自定义 URL scheme 嵌入的、由智能体控制的 Canvas 面板
title: Canvas
x-i18n:
    generated_at: "2026-04-08T06:15:02Z"
    model: gpt-5.4
    provider: openai
...
macOS 应用使用 `WKWebView` 嵌入了一个由智能体控制的 **Canvas 面板**。它是一个轻量级可视化工作区，用于承载 HTML/CSS/JS、A2UI 以及小型交互式 UI 表面。
...
Canvas 面板通过**自定义 URL scheme**来提供这些文件：
...
可在“设置”→ **Allow Canvas** 中禁用 Canvas。禁用后，canvas
...
A2UI 由 Gateway 网关的 canvas host 托管，并在 Canvas 面板内渲染。
当 Gateway 网关宣告了 Canvas host 后，macOS 应用会在首次打开时自动导航到
A2UI host 页面。
