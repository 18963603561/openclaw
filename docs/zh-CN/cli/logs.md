...
read_when:
    ...
    - 你想为工具链获取 JSON 日志行
summary: '`openclaw logs` 的 CLI 参考（通过 RPC 持续查看 Gateway 网关日志）'
title: logs
x-i18n:
    generated_at: "2026-04-08T03:52:47Z"
    model: gpt-5.4
    provider: openai
...
通过 RPC 持续查看 Gateway 网关文件日志（适用于远程模式）。
...
当你传入 `--url` 时，CLI 不会自动应用配置或环境变量中的凭证。如果目标 Gateway 网关需要身份验证，请显式传入 `--token`。
