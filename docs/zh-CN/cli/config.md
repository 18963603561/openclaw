...
- 在不支持运行时可变更的接口上，SecretRef 赋值会被拒绝（例如 `hooks.token`、`commands.ownerDisplaySecret`、Discord 线程绑定 webhook token，以及 WhatsApp creds JSON）。参见 [SecretRef ?????](/zh-CN/reference/secretref-credential-surface)。
