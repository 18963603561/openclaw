---
read_when:
    - 你正在进行首次运行设置，但不想走完整的 CLI 新手引导流程
    - 你想设置默认工作区路径
summary: '`openclaw setup` 的 CLI 参考（初始化配置 + 工作区）'
title: setup
x-i18n:
    generated_at: "2026-04-08T03:56:13Z"
    model: gpt-5.4
    provider: openai
...
- 普通的 `openclaw setup` 会初始化配置 + 工作区，而不会进入完整的新手引导流程。
- 当存在任一新手引导标志时（`--wizard`、`--non-interactive`、`--mode`、`--remote-url`、`--remote-token`），系统会自动运行新手引导。
