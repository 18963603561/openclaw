---
read_when:
    - 你想了解 `openclaw.ai/install.sh` 时
    - 你想自动化安装（CI / 无头环境）时
    - 你想从 GitHub 检出目录安装时
summary: 安装脚本（`install.sh`、`install-cli.sh`、`install.ps1`）的工作方式、标志与自动化
title: 安装器内部机制
x-i18n:
    generated_at: "2026-04-08T06:05:09Z"
    model: gpt-5.4
    provider: openai
...
OpenClaw 提供三个由 `openclaw.ai` 分发的安装脚本。

| 脚本 | 平台 | 作用 |
| ---------------------------------- | -------------------- | -------------------------------------------------------------------------------------------------------------- |
| [`install.sh`](#installsh) | macOS / Linux / WSL | 在需要时安装 Node，使用 npm（默认）或 git 安装 OpenClaw，并可运行新手引导。 |
| [`install-cli.sh`](#install-clish) | macOS / Linux / WSL | 使用 npm 或 git 检出模式，将 Node + OpenClaw 安装到本地前缀（`~/.openclaw`）中。无需 root 权限。 |
| [`install.ps1`](#installps1) | Windows（PowerShell） | 在需要时安装 Node，使用 npm（默认）或 git 安装 OpenClaw，并可运行新手引导。 |
...
<Steps>
  <Step title="检测 OS">
    支持 macOS 和 Linux（包括 WSL）。如果检测到 macOS，则会在缺少 Homebrew 时先安装它。
  </Step>
  <Step title="默认确保安装 Node.js 24">
    检查 Node 版本，并在需要时安装 Node 24（macOS 上使用 Homebrew，Linux 上的 apt/dnf/yum 使用 NodeSource 安装脚本）。出于兼容性考虑，OpenClaw 仍然支持 Node 22 LTS，目前要求 `22.14+`。
  </Step>
  ...
  <Step title="安装后任务">
    - 以尽力而为的方式刷新已加载的 gateway 服务（`openclaw gateway install --force`，然后重启）
    - 在升级和 git 安装时以尽力而为方式运行 `openclaw doctor --non-interactive`
    - 在合适情况下尝试运行新手引导（有 TTY、未禁用新手引导，并且 bootstrap/配置检查通过）
...
如果在 OpenClaw 检出目录内运行（存在 `package.json` + `pnpm-workspace.yaml`），脚本会提供以下选择：
...
| `--no-git-update` | 对已有检出目录跳过 `git pull` |
| `--no-prompt` | 禁用提示 |
| `--no-onboard` | 跳过新手引导 |
| `--onboard` | 启用新手引导 |
| `--dry-run` | 打印操作而不实际应用更改 |
...
| `OPENCLAW_GIT_UPDATE=0\|1` | 控制是否进行 git 更新 |
...
<Steps>
  <Step title="安装本地 Node 运行时">
    将一个固定且受支持的 Node LTS tarball（版本内嵌在脚本中，并独立更新）下载到 `<prefix>/tools/node-v<version>`，并校验 SHA-256。
  </Step>
  <Step title="确保已安装 Git">
    若缺少 Git，则会尝试在 Linux 上通过 apt/dnf/yum 安装，或在 macOS 上通过 Homebrew 安装。
  </Step>
...
| `OPENCLAW_GIT_UPDATE=0\|1` | 控制是否更新已有检出目录 |
| `OPENCLAW_NO_ONBOARD=1` | 跳过新手引导 |
| `OPENCLAW_NPM_LOGLEVEL=error\|warn\|notice` | npm 日志级别 |
| `SHARP_IGNORE_GLOBAL_LIBVIPS=0\|1` | 控制 sharp/libvips 行为（默认：`1`） |
...
<Steps>
  ...
  <Step title="默认确保安装 Node.js 24">
    如果缺失，会尝试通过 winget、Chocolatey，然后是 Scoop 安装。出于兼容性考虑，Node 22 LTS（目前 `22.14+`）仍受支持。
  </Step>
  <Step title="安装 OpenClaw">
...
<Note>
如果使用 `-InstallMethod git` 且缺少 Git，脚本会退出，并打印 Git for Windows 链接。
</Note>
...
请使用非交互式标志/env vars，以获得可预测的运行结果。
...
<AccordionGroup>
  <Accordion title="为什么需要 Git？">
    `git` 安装方法需要 Git。对于 `npm` 安装，也仍然会检查/安装 Git，以避免依赖项使用 git URL 时出现 `spawn git ENOENT` 失败。
  </Accordion>

  <Accordion title="为什么 npm 在 Linux 上会遇到 EACCES？">
    某些 Linux 环境会将 npm 全局前缀指向 root 拥有的路径。`install.sh` 可以把前缀切换到 `~/.npm-global`，并把 PATH 导出追加到 shell rc 文件中（前提是这些文件存在）。
  </Accordion>

  <Accordion title="sharp/libvips 问题">
    这些脚本默认设置 `SHARP_IGNORE_GLOBAL_LIBVIPS=1`，以避免 sharp 针对系统 libvips 进行构建。如果你想覆盖此行为：
  ...
  <Accordion title="安装后找不到 openclaw">
    这通常是 PATH 问题。请参阅 [Node.js 故障排除](/zh-CN/install/node#troubleshooting)。
  </Accordion>
</AccordionGroup>
