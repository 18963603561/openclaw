---
read_when:
    - 你希望在云 VPS 上 24/7 运行 OpenClaw（而不是在你的笔记本上）
    - 你希望在自己的 VPS 上运行生产级、持续在线的 Gateway 网关
    - 你希望完全控制持久化、二进制和重启行为
    - 你正在 Hetzner 或类似提供商上通过 Docker 运行 OpenClaw
summary: 在廉价 Hetzner VPS（Docker）上 24/7 运行 OpenClaw Gateway 网关，并具备持久化状态与内置二进制
title: Hetzner
x-i18n:
    generated_at: "2026-04-08T06:03:32Z"
    model: gpt-5.4
    provider: openai
...
在 Hetzner VPS 上使用 Docker 运行一个持久化的 OpenClaw Gateway 网关，并具备持久化状态、内置二进制以及安全的重启行为。

如果你想要一个“每月约 5 美元即可 24/7 运行 OpenClaw”的方案，这是最简单且可靠的配置。
Hetzner 的价格可能会变化；请选择最小的 Debian/Ubuntu VPS，如果遇到 OOM 再扩容。

安全模型提醒：

- 当所有人都处于同一个信任边界内，且运行时仅用于业务场景时，共享的公司智能体是可以接受的。
...
如果你使用的是其他 Linux VPS，请自行调整对应的软件包。
...
<Steps>
  ...
  <Step title="配置环境变量">
    ...
    **不要提交此文件。**
    ...
    已存储的 provider OAuth/API-key 认证位于已挂载的
  ...
  <Step title="Docker Compose 配置">
    ...
    services:
      openclaw-gateway:
        ...
        ports:
          # 推荐：让 VPS 上的 Gateway 网关保持仅 loopback 可访问；通过 SSH 隧道访问。
          # 若要公开暴露，请移除 `127.0.0.1:` 前缀，并相应配置防火墙。
    ...
    `--allow-unconfigured` 仅用于引导阶段的便利，它不能替代正式的 gateway 配置。你仍然应该设置认证（`gateway.auth.token` 或密码），并为你的部署使用安全的 bind 设置。
  ...
  <Step title="共享 Docker VM 运行时步骤">
    ...
    - [将所需二进制预构建到镜像中](/zh-CN/install/docker-vm-runtime#bake-required-binaries-into-the-image)
    - [构建并启动](/zh-CN/install/docker-vm-runtime#build-and-launch)
    - [各项内容持久化在哪里](/zh-CN/install/docker-vm-runtime#what-persists-where)
    - [更新](/zh-CN/install/docker-vm-runtime#updates)

  </Step>

  <Step title="Hetzner 专属访问方式">
    完成上述共享构建和启动步骤后，从你的笔记本建立隧道：
    ...
    粘贴已配置的共享密钥。本指南默认使用 gateway token；
    如果你已切换为密码认证，请改用对应密码。

  </Step>
</Steps>

共享持久化映射请参见 [Docker VM 运行时](/zh-CN/install/docker-vm-runtime#what-persists-where)。

## 基础设施即代码（Terraform）

对于偏好基础设施即代码工作流的团队，社区维护的 Terraform 配置提供了：

- 模块化 Terraform 配置以及远程状态管理
- 通过 cloud-init 实现自动化预配
...
这种方式是在上面的 Docker 配置基础上进行补充，可提供可复现部署、版本控制的基础设施，以及自动化灾难恢复。

> **注意：**由社区维护。如需反馈问题或贡献，请参阅上述仓库链接。
