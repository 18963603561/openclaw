...
title: Docker
x-i18n:
    generated_at: "2026-04-08T06:02:02Z"
    model: gpt-5.4
    provider: openai
...
Docker 是**可选的**。只有当你想要容器化的 Gateway 网关，或者想验证 Docker 流程时才使用它。

## Docker 适合我吗？

- **是**：你想要一个隔离、可随时丢弃的 Gateway 网关环境，或者想在没有本地安装的主机上运行 OpenClaw。
...
- 至少 2 GB RAM 用于镜像构建（在 1 GB 主机上，`pnpm install` 可能因 OOM 被杀死，并以退出码 137 结束）
- 足够的磁盘空间用于镜像和日志
- 如果运行在 VPS/公网主机上，请查看
...
<Steps>
  ...
  <Step title="完成新手引导">
    ...
    - 生成 Gateway 网关 token 并将其写入 `.env`
    - 通过 Docker Compose 启动 Gateway 网关

    在 setup 期间，启动前的新手引导和配置写入会直接通过
  ...
  <Step title="打开控制 UI">
    ...
    共享 secret 粘贴到设置中。setup 脚本默认会把 token 写入 `.env`；如果你将容器配置切换为 password 认证，请改用该 password。
...
| `OPENCLAW_EXTRA_MOUNTS`        | 额外的主机 bind mounts（逗号分隔的 `source:target[:opts]`） |
...
如果检查持续失败，Docker 会将容器标记为 `unhealthy`，而
编排系统可以重启或替换该容器。
...
`tailnet` / `auto`），而不是使用像 `0.0.0.0` 或 `127.0.0.1` 这样的主机别名。
</Note>

### 存储与持久化

Docker Compose 会将 `OPENCLAW_CONFIG_DIR` bind-mount 到 `/home/node/.openclaw`，
并将 `OPENCLAW_WORKSPACE_DIR` bind-mount 到 `/home/node/.openclaw/workspace`，因此这些路径在容器被替换后依然保留。
...
为了更方便地进行日常 Docker 管理，你可以安装 `ClawDock`：
...
如果你之前是通过旧的 `scripts/shell-helpers/clawdock-helpers.sh` 原始路径安装 ClawDock，请重新执行上面的安装命令，以便你的本地辅助文件跟踪新的位置。

然后使用 `clawdock-start`、`clawdock-stop`、`clawdock-dashboard` 等命令。运行
`clawdock-help` 查看所有命令。
完整辅助器指南请参见 [ClawDock](/zh-CN/install/clawdock)。
...
<AccordionGroup>
  ...
  <Accordion title="共享网络安全说明">
    ...
    命令就可以通过 `127.0.0.1` 访问 Gateway 网关。请将其视为一个共享
    信任边界。compose 配置会移除 `NET_RAW`/`NET_ADMIN`，并在 `openclaw-cli` 上启用
    `no-new-privileges`。
  </Accordion>

  <Accordion title="权限与 EACCES">
    该镜像以 `node`（uid 1000）身份运行。如果你在
    `/home/node/.openclaw` 上看到权限错误，请确保你的主机 bind mounts 归 uid 1000 所有：
  ...
  <Accordion title="更快的重建">
    请调整 Dockerfile 顺序，使依赖层可被缓存。这样除非 lockfiles 发生变化，否则不会重新运行
    `pnpm install`：
  ...
  <Accordion title="高级用户容器选项">
    默认镜像以安全优先为设计目标，并以非 root 的 `node` 身份运行。如果你想要一个功能更完整的容器：
...
共享 VM 部署步骤（包括二进制预烘焙、持久化和更新）请参见
...
（shell、文件读写等），而 Gateway 网关本身仍保留在主机上。这样你就能为不受信任或
多租户智能体会话建立一道硬隔离边界，而无需将整个 Gateway 网关容器化。
...
<AccordionGroup>
  ...
  <Accordion title="沙箱中找不到自定义工具">
    OpenClaw 使用 `sh -lc`（login shell）运行命令，这会 source
    `/etc/profile`，并可能重置 PATH。请设置 `docker.env.PATH`，在前面加入你的
    自定义工具路径，或者在 Dockerfile 中向 `/etc/profile.d/` 添加一个脚本。
  </Accordion>

  <Accordion title="镜像构建期间被 OOM 杀死（退出码 137）">
    该 VM 至少需要 2 GB RAM。请使用更大的机器规格后重试。
  </Accordion>

  <Accordion title="控制 UI 中出现 Unauthorized 或需要配对">
    获取一个新的仪表板链接，并批准浏览器设备：
  ...
  <Accordion title="Gateway 网关目标显示 ws://172.x.x.x，或 Docker CLI 中出现配对错误">
    重置 Gateway 网关模式和绑定：
...
- [ClawDock](/zh-CN/install/clawdock) — Docker Compose 社区 setup
