...
title: 更新
x-i18n:
    generated_at: "2026-04-08T06:07:53Z"
    model: gpt-5.4
    provider: openai
...
beta 标签缺失或早于最新 stable 发布时，运行时会回退到 stable/latest。若你希望一次性使用原始 npm beta dist-tag，
请使用 `--tag beta`。

渠道语义请参见 [开发渠道](/zh-CN/install/development-channels)。
...
| 渠道 | 行为 |
| -------- | ------------------------------------------------------------------------------------------------------------- |
| `stable` | 等待 `stableDelayHours`，然后在 `stableJitterHours` 范围内通过确定性抖动执行更新（分散发布节奏）。 |
| `beta` | 每隔 `betaCheckIntervalHours` 检查一次（默认：每小时），并立即应用。 |
| `dev` | 不会自动应用。请手动使用 `openclaw update`。 |

gateway 还会在启动时记录一条更新提示（可通过 `update.checkOnStart: false` 禁用）。
...
迁移配置、审计私信策略，并检查 gateway 健康状态。详情见：[Doctor](/zh-CN/gateway/doctor)
...
- 对于源码检出目录上的 `openclaw update --channel dev`，更新器会在需要时自动引导安装 `pnpm`。如果你看到 pnpm/corepack 引导错误，请手动安装 `pnpm`（或重新启用 `corepack`），然后重新运行更新。
