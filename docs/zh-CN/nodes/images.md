...
read_when:
    - 修改媒体流水线或附件
summary: 用于发送、Gateway 网关和智能体回复的图像与媒体处理规则
title: 图像与媒体支持
x-i18n:
    generated_at: "2026-04-08T06:09:27Z"
    model: gpt-5.4
    provider: openai
...
WhatsApp 渠道通过 **Baileys Web** 运行。本文档记录了当前用于发送、Gateway 网关和智能体回复的媒体处理规则。
...
- 流程：加载到 Buffer 中，检测媒体类型，并构建正确的负载：
  - **图像：** 重新调整大小并重新压缩为 JPEG（最长边 2048 px），目标为 `channels.whatsapp.mediaMaxMb`（默认：50 MB）。
  - **音频/语音/视频：** 16 MB 以内直接透传；音频会作为语音消息发送（`ptt: true`）。
  - **文档：** 其他任何内容，最大 100 MB，并在可用时保留文件名。
- WhatsApp GIF 风格播放：发送一个带有 `gifPlayback: true` 的 MP4（CLI：`--gif-playback`），这样移动端客户端会在内联模式下循环播放。
- MIME 检测优先使用 magic bytes，其次是 headers，最后是文件扩展名。
- 说明文字来自 `--message` 或 `reply.text`；允许空说明文字。
- 日志记录：非详细模式显示 `↩️`/`✅`；详细模式会包含大小和源路径/URL。
...
- 当启用了按会话划分的 Docker 沙箱时，入站媒体会被复制到沙箱工作区中，并且 `MediaPath`/`MediaUrl` 会被改写为类似 `media/inbound/<filename>` 的相对路径。
- 媒体理解（如果通过 `tools.media.*` 或共享的 `tools.media.models` 进行配置）会在模板替换之前运行，并可将 `[Image]`、`[Audio]` 和 `[Video]` 代码块插入到 `Body` 中。
  - 音频会设置 `{{Transcript}}`，并使用转录文本进行命令解析，从而让斜杠命令仍然可用。
  - 视频和图像描述会保留任何说明文字文本，以供命令解析。
  - 如果当前活动的主图像模型本身已原生支持视觉能力，OpenClaw 会跳过 `[Image]` 摘要代码块，而是将原始图像直接传递给模型。
...
- 媒体过大或不可读取 → 日志中会出现明确错误，并且该回复会被跳过。
...
- 媒体过大时会跳过理解步骤，但回复仍会携带原始正文继续通过。

## 测试说明

- 覆盖图像/音频/文档场景下的发送 + 回复流程。
- 验证图像的重新压缩（大小限制）以及音频的语音消息标志。
- 确保多媒体回复会扇出为顺序发送。
