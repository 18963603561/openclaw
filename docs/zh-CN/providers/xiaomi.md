...
title: Xiaomi MiMo
x-i18n:
    generated_at: "2026-04-08T07:01:10Z"
    model: gpt-5.4
    provider: openai
...
兼容 OpenAI 的端点，并通过 API 密钥进行认证。请先在
[Xiaomi MiMo 控制台](https://platform.xiaomimimo.com/#/console/api-keys) 创建你的 API 密钥，然后使用该密钥配置内置的 `xiaomi` 提供商。
...
- 当设置了 `XIAOMI_API_KEY`（或存在认证配置文件）时，系统会自动注入该提供商。
- 提供商规则请参见 [?????](/zh-CN/concepts/model-providers)。
