---
read_when:
    - 你想在 OpenClaw 中使用隐私优先的推理
    - 你想获取 Venice AI 设置指南
summary: 在 OpenClaw 中使用 Venice AI 的隐私优先模型
title: Venice AI
x-i18n:
    generated_at: "2026-04-08T07:00:10Z"
    model: gpt-5.4
    provider: openai
...
**Venice** 是我们重点推荐的 Venice 配置，用于实现隐私优先推理，并可选择通过匿名方式访问专有模型。

Venice AI 提供以隐私为中心的 AI 推理，支持无审查模型，并可通过其匿名代理访问主流专有模型。所有推理默认都是私有的——不会用你的数据进行训练，也不会记录日志。

## 为什么在 OpenClaw 中使用 Venice

- **私有推理**：适用于开源模型（不记录日志）。
- **无审查模型**：在你需要时可用。
- **匿名访问**：当质量更重要时，可匿名访问专有模型（Opus / GPT / Gemini）。
- 兼容 OpenAI 的 `/v1` 端点。
...
| 模式 | 描述 | 模型 |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| **Private** | 完全私有。提示词 / 响应**绝不会被存储或记录**。临时性。 | Llama、Qwen、DeepSeek、Kimi、MiniMax、Venice Uncensored 等 |
| **Anonymized** | 通过 Venice 代理并剥离元数据。底层提供商（OpenAI、Anthropic、Google、xAI）看到的是匿名化请求。 | Claude、GPT、Gemini、Grok |

## 功能

- **隐私优先**：可在 “private”（完全私有）和 “anonymized”（代理）模式之间选择
- **无审查模型**：可访问没有内容限制的模型
...
1. 提示你输入 API key（或使用现有的 `VENICE_API_KEY`）
2. 显示所有可用的 Venice 模型
3. 让你选择默认模型
4. 自动配置提供商
...
- **默认模型**：`venice/kimi-k2-5`，适合强大的私有推理 + 视觉。
- **高能力选项**：`venice/claude-opus-4-6`，是最强的匿名 Venice 路径。
- **隐私**：选择 “private” 模型可获得完全私有的推理。
- **能力**：选择 “anonymized” 模型可通过 Venice 的代理访问 Claude、GPT、Gemini。
...
| 使用场景 | 推荐模型 | 原因 |
| -------------------------- | -------------------------------- | -------------------------------------------- |
| **常规聊天（默认）** | `kimi-k2-5` | 强大的私有推理 + 视觉 |
| **整体质量最佳** | `claude-opus-4-6` | 最强的匿名 Venice 选项 |
...
| **无审查** | `venice-uncensored` | 没有内容限制 |

## 可用模型（共 41 个）

### Private 模型（26 个）—— 完全私有，无日志
...
当设置了 `VENICE_API_KEY` 时，OpenClaw 会自动从 Venice API 发现模型。如果 API 不可达，则会回退到静态目录。
...
- **Private 模型**：通常成本更低
- **Anonymized 模型**：与直接 API 定价接近，外加少量 Venice 费用

## 对比：Venice 与直接 API

| 方面 | Venice（Anonymized） | 直接 API |
| ------------ | ----------------------------- | ------------------- |
| **隐私** | 元数据被剥离，匿名化 | 关联你的账号 |
| **延迟** | +10-50ms（代理） | 直连 |
| **功能** | 支持大多数功能 | 完整功能 |
| **计费** | Venice 积分 | 提供商计费 |
...
Venice 模型目录会动态更新。运行 `openclaw models list` 查看当前可用模型。部分模型可能暂时离线。
