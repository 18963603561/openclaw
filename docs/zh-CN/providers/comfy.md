...
read_when:
    ...
    - 你想将 Comfy Cloud 与图像、视频或音乐工作流配合使用
    - 你需要内置 comfy 插件的配置键
summary: 在 OpenClaw 中设置基于 ComfyUI 工作流的图像、视频和音乐生成
title: ComfyUI
x-i18n:
    generated_at: "2026-04-08T06:51:37Z"
    model: gpt-5.4
    provider: openai
...
- 从指定节点或所有匹配的输出节点下载输出结果

这个内置插件是由工作流驱动的，因此 OpenClaw 不会尝试将通用的
`size`、`aspectRatio`、`resolution`、`durationSeconds` 或类似 TTS 的控制项映射到你的图结构中。

## 配置结构

Comfy 支持共享的顶层连接设置，以及按能力划分的工作流区段：
...
- `allowPrivateNetwork`：在云模式下允许使用私有 / 局域网 `baseUrl`

位于 `image`、`video` 或 `music` 下的按能力划分配置键：
...
当前，Comfy 视频工作流通过已配置的图结构支持文生视频和图生视频。OpenClaw 不会将输入视频传入 Comfy 工作流。

## 音乐工作流

这个内置插件为工作流定义的音频或音乐输出注册了一个音乐生成提供商，并通过共享 `music_generate` 工具对外提供：

```text
/tool music_generate prompt="Warm ambient synth loop with soft tape texture"
```

使用 `music` 配置区段指向你的音频工作流 JSON 和输出节点。
...
云模式仍使用相同的 `image`、`video` 和 `music` 工作流区段。
...
除非已配置对应的 Comfy 工作流区段，否则实时测试会跳过单独的图像、视频或音乐用例。
