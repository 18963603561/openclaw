...
title: Synthetic
x-i18n:
    generated_at: "2026-04-08T06:58:28Z"
    model: gpt-5.4
    provider: openai
...
Synthetic 暴露的是兼容 Anthropic 的端点。OpenClaw 会将其注册为
...
base URL，请覆盖 `models.providers.synthetic.baseUrl`。

## 模型目录

以下所有模型的 cost 都为 `0`（输入 / 输出 / 缓存）。
...
- 如果你启用了模型 allowlist（`agents.defaults.models`），请把你计划使用的每个模型都加入其中。
