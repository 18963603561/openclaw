...
title: 工具调用 API
x-i18n:
    generated_at: "2026-04-08T05:52:04Z"
    model: gpt-5.4
    provider: openai
...
OpenClaw 的 Gateway 网关提供了一个简单的 HTTP 端点，用于直接调用单个工具。它始终启用，并使用 Gateway 网关认证加工具策略。与兼容 OpenAI 的 `/v1/*` 接口一样，共享密钥 Bearer 认证会被视为整个 Gateway 网关的受信任操作员访问。
...
- 当 `gateway.auth.mode="trusted-proxy"` 时，HTTP 请求必须来自
  已配置的非 loopback 受信任代理来源；同主机的 local loopback 代理
  不满足此模式要求。
- 如果配置了 `gateway.auth.rateLimit` 且认证失败次数过多，端点会返回 `429`，并带有 `Retry-After`。

## 安全边界（重要）

应将此端点视为该 Gateway 网关实例的**完整操作员访问**入口。

- 此处的 HTTP Bearer 认证不是狭义的按用户范围模型。
- 此端点的有效 Gateway 网关 token/password 应被视为所有者/操作员凭据。
- 对于共享密钥认证模式（`token` 和 `password`），即使调用方发送了更窄的 `x-openclaw-scopes` 请求头，端点也会恢复正常的完整操作员默认值。
- 共享密钥认证还会将此端点上的直接工具调用视为 owner-sender 轮次。
- 受信任的携带身份 HTTP 模式（例如受信任代理认证，或私有入口上的 `gateway.auth.mode="none"`）在存在 `x-openclaw-scopes` 时会遵循它，否则会回退到正常的操作员默认作用域集合。
- 仅应在 loopback / tailnet / 私有入口上保留此端点；不要将其直接暴露到公共互联网。
...
- 受信任的携带身份 HTTP 模式（例如受信任代理认证，或私有入口上的 `gateway.auth.mode="none"`）
  ...
  - 仅当调用方显式缩小作用域且省略 `operator.admin` 时，才会失去 owner 语义
...
- `action`（字符串，可选）：如果工具 schema 支持 `action` 且 args 负载中未提供它，则会映射进 args。
- `args`（对象，可选）：工具专属参数。
...
- Exec 审批是操作员保护措施，不是此 HTTP 端点的独立授权边界。如果某个工具可通过 Gateway 网关认证 + 工具策略从这里访问，`/tools/invoke` 不会额外添加每次调用审批提示。
- 不要与不受信任的调用方共享 Gateway 网关 Bearer 凭据。如果你需要跨信任边界隔离，请运行独立的 Gateway 网关（最好也使用独立的操作系统用户/主机）。

Gateway 网关 HTTP 默认还会应用一个硬拒绝列表（即使会话策略允许该工具也一样）：

- `exec` —— 直接命令执行（RCE 攻击面）
- `spawn` —— 任意子进程创建（RCE 攻击面）
- `shell` —— shell 命令执行（RCE 攻击面）
- `fs_write` —— 对宿主机文件进行任意修改
- `fs_delete` —— 删除宿主机上的任意文件
- `fs_move` —— 任意移动/重命名宿主机文件
- `apply_patch` —— 补丁应用可以重写任意文件
- `sessions_spawn` —— 会话编排；远程派生智能体属于 RCE
- `sessions_send` —— 跨会话消息注入
- `cron` —— 持久化自动化控制平面
- `gateway` —— Gateway 网关控制平面；防止通过 HTTP 重新配置
- `nodes` —— 节点命令中继可触达已配对主机上的 `system.run`
- `whatsapp_login` —— 需要终端二维码扫描的交互式设置；会在 HTTP 上挂起
