...
`~/.openclaw/openclaw.json` 的核心配置参考。若需按任务组织的概览，请参阅 [??](/zh-CN/gateway/configuration)。
...
- [Memory ????](/zh-CN/reference/memory-config) 用于 `agents.defaults.memorySearch.*`、`memory.qmd.*`、`memory.citations`，以及 `plugins.entries.memory-core.config.dreaming` 下的 dreaming 配置
- [????](/zh-CN/tools/slash-commands) 用于当前内置 + 内置插件命令目录
...
- 顶层 `bindings[]` 中 `type: "acp"` 的条目会为论坛话题配置持久 ACP 绑定（在 `match.peer.id` 中使用规范的 `chatId:topic:topicId`）。字段语义与 [ACP ???](/zh-CN/tools/acp-agents#channel-specific-settings) 共享。
...
- 顶层 `bindings[]` 中 `type: "acp"` 的条目会为渠道和线程配置持久 ACP 绑定（在 `match.peer.id` 中使用渠道/线程 ID）。字段语义与 [ACP ???](/zh-CN/tools/acp-agents#channel-specific-settings) 共享。
...
- 顶层 `bindings[]` 中 `type: "acp"` 的条目可将 BlueBubbles 会话绑定到持久 ACP 会话。在 `match.peer.id` 中使用 BlueBubbles handle 或目标字符串（`chat_id:*`、`chat_guid:*`、`chat_identifier:*`）。共享字段语义： [ACP ???](/zh-CN/tools/acp-agents#channel-specific-settings)。
...
- 顶层 `bindings[]` 中 `type: "acp"` 的条目可将 iMessage 会话绑定到持久 ACP 会话。在 `match.peer.id` 中使用规范化 handle 或显式聊天目标（`chat_id:*`、`chat_guid:*`、`chat_identifier:*`）。共享字段语义： [ACP ???](/zh-CN/tools/acp-agents#channel-specific-settings)。
...
- 此块用于配置命令表面。当前内置 + 内置插件命令目录见 [????](/zh-CN/tools/slash-commands)。
- 本页是**配置键参考**，不是完整命令目录。渠道/插件自有命令，如 QQ Bot `/bot-ping` `/bot-help` `/bot-logs`、LINE `/card`、device-pair `/pair`、memory `/dreaming`、phone-control `/phone` 和 Talk `/voice`，记录在各自的渠道/插件页面以及 [????](/zh-CN/tools/slash-commands) 中。
- 文本命令必须是带前导 `/` 的**独立**消息。
...
- 命令文档映射：
  - 内置 + 内置插件目录：[????](/zh-CN/tools/slash-commands)
  - 渠道特定命令表面：[??](/zh-CN/channels)
  - QQ Bot 命令：[QQ Bot](/zh-CN/channels/qqbot)
  - 配对命令：[??](/zh-CN/channels/pairing)
  - LINE card 命令：[LINE](/zh-CN/channels/line)
  - memory dreaming：[Dreaming（实验性）](/zh-CN/concepts/dreaming)
...
- `params` 合并优先级（配置）：`agents.defaults.params`（全局基础）会被 `agents.defaults.models["provider/model"].params`（每模型）覆盖，然后 `agents.list[].params`（匹配的智能体 ID）会按键覆盖。详见 [提示词缓存](/zh-CN/reference/prompt-caching)。
...
- `mode`：`default` 或 `safeguard`（针对长历史的分块摘要）。参见 [??](/zh-CN/concepts/compaction)。
- `provider`：已注册的 compaction provider 插件 ID。设置后，将调用该 provider 的 `summarize()`，而不是使用内置 LLM 摘要。失败时回退到内置方式。设置 provider 会强制 `mode: "safeguard"`。参见 [??](/zh-CN/concepts/compaction)。
...
行为细节见 [会话修剪](/zh-CN/concepts/session-pruning)。
...
行为和分块细节见 [流式传输和分块](/zh-CN/concepts/streaming)。
...
在一个 Gateway 网关内运行多个隔离智能体。参见 [??????](/zh-CN/concepts/multi-agent)。
...
优先级细节见 [多智能体沙箱与工具](/zh-CN/tools/multi-agent-sandbox-tools)。
...
参见 [本地模型](/zh-CN/gateway/local-models)。简而言之：在足够强的硬件上通过 LM Studio Responses API 运行大型本地模型；同时保留已合并的托管模型作为回退。
...
- `plugins.entries.memory-core.config.dreaming`：memory dreaming（实验性）设置。有关阶段和阈值，见 [Dreaming（实验性）](/zh-CN/concepts/dreaming)。
  - `enabled`：dreaming 主开关（默认 `false`）。
  - `frequency`：每次完整 dreaming 扫描的 cron 频率（默认 `"0 3 * * *"`）。
  - 阶段策略和阈值属于实现细节（不是面向用户的配置键）。
- 完整 memory 配置见 [Memory ????](/zh-CN/reference/memory-config)：
