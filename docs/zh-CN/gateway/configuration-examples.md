...
read_when:
    ...
    - 查找配置示例
    - 首次设置 OpenClaw
summary: 符合 Schema 的常见 OpenClaw 配置示例
title: 配置示例
x-i18n:
    generated_at: "2026-04-08T04:25:00Z"
    model: gpt-5.4
    provider: openai
...
下面的示例与当前配置 schema 保持一致。要查看完整参考和各字段说明，请参阅 [配置](/zh-CN/gateway/configuration)。
...
保存到 `~/.openclaw/openclaw.json`，然后你就可以用这个号码向机器人发送私信。
...
如果有多个人可以向你的机器人发送私信（`allowFrom` 中有多个条目、为多个人批准配对，或 `dmPolicy: "open"`），请启用 **安全私信模式**，这样不同发送者发来的私信默认不会共享同一个上下文：
...
对于 Discord、Slack、Google Chat、Microsoft Teams、Mattermost 和 IRC，发送者授权默认首先基于 ID。
只有在你明确接受该风险的情况下，才通过各渠道的 `dangerouslyAllowNameMatching: true` 启用基于可直接变更的名称 / 邮箱 / 昵称的匹配。
