...
x-i18n:
    generated_at: "2026-04-08T04:31:40Z"
    model: gpt-5.4
    provider: openai
...
OpenClaw.app 使用 SSH 隧道连接到远程 Gateway 网关。本指南将向你展示如何完成设置。
...
远程客户端配置应使用 `gateway.remote.token` / `gateway.remote.password`。
...
兼容性说明：如果存在遗留的 `com.openclaw.ssh-tunnel` LaunchAgent，请将其移除。
...
OpenClaw.app 会连接到你客户端机器上的 `ws://127.0.0.1:18789`。SSH 隧道会将该连接转发到远程机器上的 18789 端口，也就是 Gateway 网关运行的位置。
