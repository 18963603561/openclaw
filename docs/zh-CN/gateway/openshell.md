...
- 已安装 `openshell` CLI，并且可在 `PATH` 中找到（或者通过
...
2. 重启 Gateway 网关。下一次智能体轮次运行时，OpenClaw 会创建一个 OpenShell
   沙箱，并通过它来路由工具执行。
...
- 文件工具仍通过沙箱桥接运行，但在每次轮次之间，本地工作区
  仍然是事实上的来源。

最适合：

- 你会在 OpenClaw 之外本地编辑文件，并希望这些更改自动在
  沙箱中可见。
- 你希望 OpenShell 沙箱的行为尽可能接近 Docker 后端。
- 你希望每次 `exec` 轮次之后，宿主机工作区能够反映沙箱中的写入结果。
...
- 当沙箱首次创建时，OpenClaw 会将本地工作区一次性初始化到
  远程工作区中。
...
重要说明：如果在初次初始化之后，你又在 OpenClaw 之外于宿主机上编辑文件，
远程沙箱**不会**看到这些更改。请使用
`openclaw sandbox recreate` 重新初始化。
...
| **每轮次开销**           | 更高（上传 + 下载）       | 更低（直接远程操作）      |
| **本地编辑可见吗？**     | 可以，在下一次 exec 时可见 | 不行，除非 recreate       |
...
对于 `remote` 模式，**recreate 尤其重要**：它会删除该作用域下的权威
远程工作区。下一次使用时，会从
本地工作区重新初始化一个新的远程工作区。
...
- `sandbox.docker.*` 下特定于 Docker 的运行时选项仅适用于 Docker
  后端。

## 工作原理

1. OpenClaw 调用 `openshell sandbox create`（按配置带上 `--from`、`--gateway`、
   `--policy`、`--providers`、`--gpu` 参数）。
2. OpenClaw 调用 `openshell sandbox ssh-config <name>` 获取该沙箱的 SSH 连接
   详情。
3. 核心层将 SSH 配置写入临时文件，并使用与通用 SSH 后端相同的
   远程文件系统桥接方式打开 SSH 会话。
4. 在 `mirror` 模式下：在 exec 前将本地同步到远程，运行后再同步回本地。
