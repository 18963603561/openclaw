...
<CardGroup cols={2}>
  ...
  <Card title="Secrets 计划契约" icon="shield-check" href="/zh-CN/gateway/secrets-plan-contract">
...
参见：[????](/zh-CN/gateway/remote)、[凭证](/zh-CN/gateway/authentication)、[Tailscale](/zh-CN/gateway/tailscale)。
...
完整协议文档见：[Gateway ????](/zh-CN/gateway/protocol)。
...
如需完整诊断步骤，请使用 [故障排除](/zh-CN/gateway/troubleshooting)。
