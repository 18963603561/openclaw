...
> 此内容已合并到 [Network](/network#core-model)。当前指南请参阅该页面。

大多数操作都通过 Gateway 网关（`openclaw gateway`）进行，它是一个长期运行的单一进程，负责持有渠道连接和 WebSocket 控制平面。

## 核心规则

- 建议每台主机运行一个 Gateway 网关。它是唯一允许持有 WhatsApp Web 会话的进程。对于救援机器人或严格隔离场景，可运行多个使用独立 profile 和端口的 Gateway 网关。参见 [Multiple gateways](/zh-CN/gateway/multiple-gateways)。
...
- Canvas host 由 Gateway 网关 HTTP 服务器通过与 Gateway 网关**相同端口**提供服务（默认 `18789`）：
  - `/__openclaw__/canvas/`
  - `/__openclaw__/a2ui/`
    当配置了 `gateway.auth` 且 Gateway 网关绑定超出 loopback 范围时，这些路由会受到 Gateway 网关认证保护。节点客户端使用与其当前 WS 会话绑定的节点作用域 capability URL。参见 [Gateway configuration](/zh-CN/gateway/configuration)（`canvasHost`、`gateway`）。
- 远程使用通常依赖 SSH 隧道或 tailnet VPN。参见 [Remote access](/zh-CN/gateway/remote) 和 [Discovery](/zh-CN/gateway/discovery)。
