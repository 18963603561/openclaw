---
read_when:
    - 故障排除中心已将你引导到这里进行更深入的诊断
    - 你需要按稳定症状划分并附带精确命令的操作手册章节
summary: 针对 Gateway 网关、渠道、自动化、节点和浏览器的深度故障排除操作手册
title: 故障排除
x-i18n:
    generated_at: "2026-04-08T05:54:43Z"
    model: gpt-5.4
    provider: openai
...
- `openclaw channels status --probe` 显示实时的按账号划分传输状态，并且在支持的情况下显示 probe/audit 结果，例如 `works` 或 `audit ok`。
...
- 很小的直接调用成功，但 OpenClaw 运行只在较大提示词下失败
- 后端报错 `messages[].content` 期望字符串
- 后端崩溃仅在较大 prompt token 数或完整智能体运行时提示词下出现
...
- 很小的直接请求成功，但 OpenClaw 智能体运行因后端/模型崩溃而失败（例如某些 `inferrs` 构建中的 Gemma）→ OpenClaw 传输层很可能已经是正确的；失败的是更大的智能体运行时提示词形状对应的后端。
- 禁用工具后失败有所减少但未消失 → 工具 schema 确实增加了压力，但剩余问题仍然是上游模型/服务器容量限制或后端缺陷。
...
3. 在可能的情况下降低提示词压力：更小的工作区 bootstrap、更短的会话历史、更轻量的本地模型，或选择长上下文支持更强的后端。
4. 如果很小的直接请求持续通过，而 OpenClaw 智能体轮次仍在后端内部崩溃，请将其视为上游服务器/模型限制，并向对应上游提交包含已接受负载形状的复现。
...
如果渠道已连通但没有任何响应，请在重新连接任何内容之前先检查路由和策略。
...
- 正确的 probe URL 和仪表板 URL。
- 客户端与 Gateway 网关之间的认证模式/token 是否匹配。
- 在要求设备身份时是否使用了 HTTP。

常见特征：

- `device identity required` → 非安全上下文或缺少设备认证。
- `origin not allowed` → 浏览器 `Origin` 不在 `gateway.controlUi.allowedOrigins` 中（或你正从非 loopback 的浏览器 origin 连接，但未显式加入 allowlist）。
...
- 带有 `canRetryWithDeviceToken=true` 的 `AUTH_TOKEN_MISMATCH` → 客户端可以使用缓存的设备 token 进行一次受信任重试。
- 该缓存 token 重试会复用与已配对设备 token 一起存储的缓存 scope 集合。显式 `deviceToken` / 显式 `scopes` 调用方则保留自己请求的 scope 集合。
- 除该重试路径外，连接认证优先级为：显式共享 token/password 优先，其次是显式 `deviceToken`，然后是已存储设备 token，最后是 bootstrap token。
- 在异步 Tailscale Serve 控制 UI 路径上，对于相同 `{scope, ip}` 的失败尝试，会先串行处理，然后限流器才记录失败。因此，同一客户端的两次错误并发重试，第二次可能显示 `retry later`，而不是两个普通的不匹配错误。
- 来自浏览器 origin loopback 客户端的 `too many failed authentication attempts (retry later)` → 来自同一规范化 `Origin` 的重复失败会被临时锁定；另一个 localhost origin 会使用单独的桶。
- 此后重复出现的 `unauthorized` 重试 → 共享 token/设备 token 已漂移；请刷新 token 配置，并在需要时重新批准/轮换设备 token。
...
如果日志显示 nonce/signature 错误，请更新连接中的客户端并验证其是否：

1. 等待 `connect.challenge`
2. 对绑定到 challenge 的负载进行签名
3. 使用相同的 challenge nonce 发送 `connect.params.device.nonce`

如果 `openclaw devices rotate` / `revoke` / `remove` 被意外拒绝：

- 已配对设备 token 会话只能管理**其自己的**设备，除非调用方同时具有 `operator.admin`
...
当 `openclaw gateway probe` 已探测到某个目标，但仍然打印警告块时使用本节。
...
- 该警告是否与 SSH 回退、多个 Gateway 网关、缺少 scopes 或未解析认证引用有关。

常见特征：

- `SSH tunnel failed to start; falling back to direct probes.` → SSH 设置失败，但命令仍尝试了直接探测已配置/loopback 目标。
- `multiple reachable gateways detected` → 有多个目标响应。通常这表示有意的多 Gateway 网关部署，或存在陈旧/重复监听器。
- `Probe diagnostics are limited by gateway scopes (missing operator.read)` → 连接成功，但详细 RPC 受 scope 限制；请配对设备身份或使用具有 `operator.read` 的凭证。
- 未解析的 `gateway.auth.*` / `gateway.remote.*` SecretRef 警告文本 → 该失败目标在此命令路径中无法获取认证材料。
...
如果渠道状态显示已连接，但消息流已停止，请重点检查策略、权限和渠道特定的传递规则。
...
如果 cron 或 heartbeat 未运行，或未成功传递，请先验证调度器状态，再检查传递目标。
...
- 带有 `reason=empty-heartbeat-file` 的 `heartbeat skipped` → `HEARTBEAT.md` 存在，但只包含空行 / Markdown 标题，因此 OpenClaw 会跳过模型调用。
- 带有 `reason=no-tasks-due` 的 `heartbeat skipped` → `HEARTBEAT.md` 包含 `tasks:` 代码块，但本次 tick 没有任何任务到期。
- `heartbeat: unknown accountId` → heartbeat 传递目标的账号 id 无效。
- 带有 `reason=dm-blocked` 的 `heartbeat skipped` → heartbeat 目标被解析为私信风格目的地，而 `agents.defaults.heartbeat.directPolicy`（或每智能体覆盖）被设置为 `block`。
...
当 Gateway 网关本身健康，但浏览器工具操作失败时使用本节。
...
- `Browser attachOnly is enabled ... not reachable` 或 `Browser attachOnly is enabled and CDP websocket ... is not reachable` → attach-only profile 没有可达目标，或者 HTTP 端点虽有响应，但 CDP WebSocket 仍无法打开。
- `Playwright is not available in this gateway build; '<feature>' is unsupported.` → 当前 Gateway 网关安装缺少完整的 Playwright 包；ARIA 快照和基础页面截图仍可使用，但导航、AI 快照、基于 CSS 选择器的元素截图和 PDF 导出仍不可用。
...
- `existing-session file uploads currently support one file at a time.` → 在 Chrome MCP profiles 中每次调用只发送一个上传文件。
...
- attach-only 或远程 CDP profiles 上存在过期的 viewport / dark-mode / locale / offline 覆盖状态 → 运行 `openclaw browser stop --browser-profile <name>` 以关闭当前控制会话并释放 Playwright/CDP 仿真状态，而无需重启整个 Gateway 网关。
...
大多数升级后的故障都源于配置漂移，或当前强制执行了更严格的默认值。
...
- 如果 `gateway.mode=remote`，CLI 调用可能正在指向远程，而你的本地服务本身是正常的。
- 显式 `--url` 调用不会回退到已存储凭证。
...
- `refusing to bind gateway ... without auth` → 非 loopback 绑定但无有效 Gateway 网关认证路径。
- `RPC probe: failed` 而运行时仍在运行 → Gateway 网关存活，但当前 auth/url 无法访问。
...
如果检查后服务配置和运行时仍然不一致，请从同一 profile/state 目录重新安装服务元数据：
