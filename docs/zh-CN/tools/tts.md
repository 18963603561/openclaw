...
当前内置的 Microsoft 语音提供商通过 `node-edge-tts` 库使用 Microsoft Edge 的在线神经 TTS 服务。这是一项托管服务（不是本地服务），使用 Microsoft 端点，并且不需要 API 密钥。
`node-edge-tts` 暴露了语音配置选项和输出格式，但并非所有选项都受该服务支持。旧版配置和使用 `edge` 的指令输入仍然有效，并会被规范化为 `microsoft`。

由于这一路径属于没有公开 SLA 或配额说明的公共 Web 服务，请将其视为尽力而为方案。如果你需要有保障的限制和支持，请使用 OpenAI 或 ElevenLabs。
...
如果配置了多个提供商，则会先使用当前选定提供商，其他提供商作为回退选项。
...
不。自动 TTS 默认**关闭**。可在配置中通过
`messages.tts.auto` 启用，或在每个会话中使用 `/tts always`（别名：`/tts on`）启用。
...
- `provider`：语音提供商 id，例如 `"elevenlabs"`、`"microsoft"`、`"minimax"` 或 `"openai"`（回退自动处理）。
- 如果**未**设置 `provider`，OpenClaw 会按注册表自动选择顺序使用第一个已配置的语音提供商。
...
- `maxTextLength`：TTS 输入的硬上限（字符数）。如果超过，`/tts audio` 会失败。
- `timeoutMs`：请求超时（毫秒）。
...
- `providers.openai.baseUrl`：覆盖 OpenAI TTS 端点。
  ...
  - 非默认值会被视为兼容 OpenAI 的 TTS 端点，因此接受自定义 model 和 voice 名称。
...
- `providers.microsoft.outputFormat`：Microsoft 输出格式（例如 `audio-24khz-48kbitrate-mono-mp3`）。
  - 有效值请参见 Microsoft Speech 输出格式；内置的 Edge 支持传输并不支持所有格式。
...
当 `messages.tts.auto` 为 `tagged` 时，必须依赖这些指令才能触发音频。

启用后，模型可发出 `[[tts:...]]` 指令，为单次回复覆盖语音，
还可选地使用 `[[tts:text]]...[[/tts:text]]` 块提供表现性标签（笑声、歌唱提示等），这些内容应只出现在音频中。
...
- `voice`（OpenAI voice）或 `voiceId`（ElevenLabs / MiniMax）
- `model`（OpenAI TTS model、ElevenLabs model id 或 MiniMax model）
...
Slash commands 会将本地覆盖项写入 `prefsPath`（默认：
...
- 跳过过短回复（少于 10 个字符）。
- 启用时使用 `agents.defaults.model.primary`（或 `summaryModel`）对长回复做摘要。
- 将生成的音频附加到回复中。
...
- 命令需要已授权发送者（仍然适用允许列表 / 所有者规则）。
- 必须启用 `commands.text` 或原生命令注册。
- `off|always|inbound|tagged` 是按会话生效的切换项（`/tts on` 是 `/tts always` 的别名）。
- `limit` 和 `summary` 存储在本地 prefs 中，而不是主配置中。
- `/tts audio` 会生成一次性音频回复（不会打开 TTS 开关）。
- `/tts status` 会包含最近一次尝试的回退可见性：
  - 成功回退：`Fallback: <primary> -> <used>` 加 `Attempts: ...`
  - 失败：`Error: ...` 加 `Attempts: ...`
  - 详细诊断：`Attempt details: provider:outcome(reasonCode) latency`
- OpenAI 和 ElevenLabs 的 API 失败现在会包含已解析的提供商错误详情和请求 id（当提供商返回时），并会暴露在 TTS 错误 / 日志中。

## 智能体工具

`tts` 工具将文本转换为语音，并返回一个用于回复投递的音频附件。当渠道为 Feishu、Matrix、Telegram 或 WhatsApp 时，音频会以语音消息形式投递，而不是文件附件。
