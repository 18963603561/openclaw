...
title: Web 搜索
x-i18n:
    generated_at: "2026-04-09T01:04:20Z"
    model: gpt-5.4
    provider: openai
...
`web_search` 工具会使用你配置的提供商搜索网页，
并返回结果。结果会按查询缓存 15 分钟（可配置）。

OpenClaw 还包含用于 X（原 Twitter）帖子搜索的 `x_search`，
以及用于轻量级 URL 抓取的 `web_fetch`。在当前阶段，`web_fetch` 保持为
本地工具，而 `web_search` 和 `x_search` 底层可以使用 xAI Responses。
...
<Steps>
  <Step title="选择一个提供商">
    选择一个提供商并完成所需设置。有些提供商
    无需密钥，而另一些则使用 API 密钥。详见下方各提供商页面。
  </Step>
  <Step title="配置">
    ```bash
    openclaw configure --section web
    ```
    这会保存提供商以及所需凭证。对于基于 API 的
    提供商，你也可以设置环境变量（例如 `BRAVE_API_KEY`）并跳过这一步。
  </Step>
  <Step title="开始使用">
    现在智能体即可调用 `web_search`：
...
<CardGroup cols={2}>
  ...
  <Card title="Grok" icon="zap" href="/zh-CN/tools/grok-search">
    通过 xAI 网络 grounding 提供带引用的 AI 综合答案。
  </Card>
  ...
  <Card title="Ollama Web Search" icon="globe" href="/zh-CN/tools/ollama-search">
    通过你配置的 Ollama 宿主进行无需密钥的搜索。需要执行 `ollama signin`。
  </Card>
...
如果启用了原生 Codex 搜索，但当前模型不具备 Codex 能力，OpenClaw 会保持正常的托管 `web_search` 行为。
...
8. **Exa Search** -- `EXA_API_KEY` 或 `plugins.entries.exa.config.webSearch.apiKey`（顺序 65）
9. **Tavily** -- `TAVILY_API_KEY` 或 `plugins.entries.tavily.config.webSearch.apiKey`（顺序 70）

之后是无需密钥的回退方案：

10. **DuckDuckGo** -- 无需账号或 API 密钥的基于 HTML 的回退方案（顺序 100）
11. **Ollama Web 搜索** -- 通过你配置的 Ollama 宿主进行无需密钥的回退；要求 Ollama 可访问，且已通过 `ollama signin` 登录；如果宿主需要认证，也可复用 Ollama 提供商 bearer 认证（顺序 110）
12. **SearXNG** -- `SEARXNG_BASE_URL` 或 `plugins.entries.searxng.config.webSearch.baseUrl`（顺序 200）

如果未检测到任何提供商，则会回退到 Brave（你会收到缺少密钥的
错误提示，要求你进行配置）。

<Note>
  所有提供商密钥字段都支持 SecretRef 对象。在自动检测模式下，
  OpenClaw 只会解析已选中提供商的密钥 —— 未选中的 SecretRef
  会保持未激活状态。
</Note>
...
- 或省略该字段，让 OpenClaw 从可用凭证中自动检测第一个就绪的 `web_fetch`
  提供商
- 当前内置的 `web_fetch` 提供商是 Firecrawl，配置位于
...
- 默认的 Kimi 网络搜索模型（默认值为 `kimi-k2.5`）
...
OpenClaw 也可以提供使用同一个密钥进行可选 `x_search` 设置。
这是 Grok 路径中的一个单独后续步骤，而不是一个单独的顶层
网络搜索提供商选项。如果你选择其他提供商，OpenClaw 不会
显示 `x_search` 提示。
...
<Warning>
  ...
  Gemini、Grok 和 Kimi 返回的是一个带引用的综合答案。它们
  ...
  SearXNG 仅对可信私有网络或 loopback 宿主接受 `http://`；
...
可选结构化过滤器。OpenClaw 仅会在提供该工具调用的请求上启用内置的 xAI `x_search`
工具。

<Note>
  xAI 文档说明 `x_search` 支持关键词搜索、语义搜索、用户
  搜索和线程抓取。对于每条帖子的互动统计，例如转发、
  回复、书签或浏览量，优先对精确帖子 URL
  或状态 ID 进行定向查询。宽泛的关键词搜索可能能找到正确帖子，但返回的
  单帖级元数据不够完整。一个好的模式是：先定位帖子，再执行第二次
  聚焦到该精确帖子的 `x_search` 查询。
</Note>
...
| `enable_image_understanding` | 让 xAI 检查匹配帖子所附带的图像 |
| `enable_video_understanding` | 让 xAI 检查匹配帖子所附带的视频 |
...
如果你使用工具配置文件或 allowlists，请添加 `web_search`、`x_search` 或 `group:web`：
