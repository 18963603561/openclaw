---
read_when:
    - 你希望智能体将代码或 Markdown 编辑展示为 Diffs
    - 你希望获得可用于 canvas 的查看器 URL 或已渲染的 diff 文件
    - 你需要具备安全默认值、受控且临时的 diff 产物
summary: 面向智能体的只读 Diffs 查看器和文件渲染器（可选插件工具）
title: Diffs
x-i18n:
    generated_at: "2026-04-09T00:55:09Z"
    model: gpt-5.4
    provider: openai
...
`diffs` 是一个可选插件工具，带有简短的内置系统引导，以及一个配套 Skill，可将变更内容转换为供智能体使用的只读 diff 产物。
...
启用后，该插件会将简洁的使用说明预置到系统提示空间中，同时也会暴露一个详细 Skill，供智能体在需要更完整说明时使用。

## 快速开始

1. 启用插件。
2. 在以 canvas 为主的流程中，以 `mode: "view"` 调用 `diffs`。
3. 在聊天文件投递流程中，以 `mode: "file"` 调用 `diffs`。
4. 当你同时需要这两种产物时，以 `mode: "both"` 调用 `diffs`。
...
如果你希望保持 `diffs` 工具启用，但禁用其内置的系统提示引导，请将 `plugins.entries.diffs.hooks.allowPromptInjection` 设置为 `false`：
...
- `ttlSeconds`（`number`）：查看器和独立文件输出的产物 TTL，单位为秒。默认 1800，最大 21600。
...
- `patch` 与 `before` 或 `after` 一起提供时会被拒绝。
...
- `fileCount`
- `mode`
- `context`（如可用则包含 `agentId`、`sessionId`、`messageChannel`、`agentAccountId`）
...
- `mode: "both"`：返回查看器字段加文件字段。如果文件渲染失败，查看器仍会返回，并带有 `fileError` 以及兼容别名 `imageError`。

## 折叠的未变更区段

- 查看器可以显示类似 `N unmodified lines` 的行。
- 这些行上的展开控件是有条件的，并不保证对每种输入类型都存在。
- 当渲染后的 diff 具有可展开的上下文数据时，会出现展开控件，这在 before / after 输入中很常见。
- 对于许多统一 patch 输入，被省略的上下文主体并不存在于解析后的 patch hunk 中，因此该行可能会出现但没有展开控件。这属于预期行为。
- `expandUnchanged` 仅在存在可展开上下文时生效。

## 插件默认值

可在 `~/.openclaw/openclaw.json` 中设置插件级默认值：
...
- `security.allowRemoteViewer`（`boolean`，默认 `false`）
  ...
  - `true`：如果带 token 的路径有效，则允许远程查看器访问。
...
查看器文档会相对于查看器 URL 解析这些资源，因此可选的 `baseUrl` 路径前缀也会对这些资源请求生效。

URL 构造行为：

- 如果工具调用提供了 `baseUrl`，则在严格校验后使用它。
...
- 默认仅限 loopback。
- 使用带 token 的查看器路径，并对 ID 和 token 做严格校验。
- 查看器响应 CSP：
  - `default-src 'none'`
  - scripts 和 assets 仅允许来自 self
  - 不允许出站 `connect-src`
- 当启用远程访问时，对远程未命中进行限流：
  - 每 60 秒 40 次失败
...
可通过安装 Chrome、Chromium、Edge 或 Brave，或设置以上任一可执行文件路径选项来修复。
...
- `Provide either patch or before/after input, not both.`
  - 不要混用输入模式。
- `Invalid baseUrl: ...`
  - 请使用带可选路径的 `http(s)` 源，且不要带 query/hash。
...
- 默认情况下，查看器 URL 会解析到 `127.0.0.1`。
- 对于远程访问场景，请选择以下其一：
  - 设置插件 `viewerBaseUrl`，或
  - 在每次工具调用中传入 `baseUrl`，或
  - 使用 `gateway.bind=custom` 和 `gateway.customBindHost`
- 如果 `gateway.trustedProxies` 包含同主机代理的 loopback（例如 Tailscale Serve），那么没有转发客户端 IP 头的原始 loopback 查看器请求会按设计以封闭失败方式被拒绝。
- 对于这种代理拓扑：
  - 如果你只需要附件，优先使用 `mode: "file"` 或 `mode: "both"`，或者
  - 当你需要可分享的查看器 URL 时，有意启用 `security.allowRemoteViewer`，并设置插件 `viewerBaseUrl`，或传入代理 / 公网 `baseUrl`
- 只有在你确实打算开放外部查看器访问时，才启用 `security.allowRemoteViewer`。

未变更行没有展开按钮：

- 当 patch 输入不携带可展开上下文时，可能会出现这种情况。
- 这是预期行为，不表示查看器失败。
...
- 对于需要附件的外部聊天渠道，优先使用 `mode: "file"`。
- 除非你的部署需要远程查看器 URL，否则请保持 `allowRemoteViewer` 关闭。
- 对敏感 Diffs，请显式设置较短的 `ttlSeconds`。
- 如果没有必要，请避免在 diff 输入中发送密钥。
