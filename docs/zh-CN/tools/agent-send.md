...
title: 智能体发送
x-i18n:
    generated_at: "2026-04-08T07:15:41Z"
    model: gpt-5.4
    provider: openai
...
`openclaw agent` 可直接从命令行运行单次智能体轮次，而无需
入站聊天消息。可用于脚本工作流、测试以及
程序化投递。
...
| `--to \<dest\>` | 从目标（电话号码、聊天 id）推导会话键 |
| `--agent \<id\>` | 指定某个已配置智能体（使用其 `main` 会话） |
| `--session-id \<id\>` | 通过 id 复用现有会话 |
| `--local` | 强制使用本地嵌入式运行时（跳过 Gateway 网关） |
| `--deliver` | 将回复发送到某个聊天渠道 |
...
- 默认情况下，CLI 会**通过 Gateway 网关**运行。添加 `--local` 可强制使用
  当前机器上的嵌入式运行时。
- 如果 Gateway 网关不可达，CLI 会**回退**到本地嵌入式运行。
- 会话选择：`--to` 会推导会话键（群组 / 渠道目标会保留隔离；直接聊天会折叠到 `main`）。
