...
关于发现/配置请参见 [??](/zh-CN/tools/plugin)，关于这些 Skills 所教授的
...
- 更广泛的威胁模型和检查清单，请参见 [??](/zh-CN/gateway/security)。
...
完整配置 schema 请参见 [Skills ??](/zh-CN/tools/skills-config)。
...
- [Skills ??](/zh-CN/tools/skills-config) —— Skill 配置参考
- [????](/zh-CN/tools/slash-commands) —— 所有可用的 slash commands
- [??](/zh-CN/tools/plugin) —— 插件系统概览
