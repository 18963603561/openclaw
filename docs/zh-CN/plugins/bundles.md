---
read_when:
    - 你想安装兼容 Codex、Claude 或 Cursor 的 bundle
    - 你需要了解 OpenClaw 如何将 bundle 内容映射到原生功能中
    - 你正在调试 bundle 检测或缺失的能力
summary: 将 Codex、Claude 和 Cursor bundle 安装并作为 OpenClaw 插件使用
title: 插件 Bundles
x-i18n:
    generated_at: "2026-04-08T06:41:26Z"
    model: gpt-5.4
    provider: openai
...
OpenClaw 可以从三个外部生态安装插件：**Codex**、**Claude** 和 **Cursor**。这些被称为 **bundles** —— 内容与元数据打包集合，OpenClaw 会将其映射到原生功能，例如 skills、hooks 和 MCP 工具。

<Info>
  Bundles **并不等同于**原生 OpenClaw 插件。原生插件在进程内运行，并且可以注册任意能力。Bundles 是内容包，具有选择性的功能映射和更窄的信任边界。
</Info>

## 为什么需要 bundles

许多有用的插件是以 Codex、Claude 或 Cursor 格式发布的。OpenClaw 不要求作者将它们重写为原生 OpenClaw 插件，而是检测这些格式，并将其支持的内容映射到原生功能集。这意味着你可以安装一个 Claude 命令包或一个 Codex skill bundle，并立即使用它。
...
<Steps>
  ...
  <Step title="验证检测结果">
    ...
    Bundles 会显示为 `Format: bundle`，其 subtype 为 `codex`、`claude` 或 `cursor`。
...
## OpenClaw 会从 bundles 映射哪些内容

并非 bundle 的所有功能当前都能在 OpenClaw 中运行。下面列出当前可用的内容，以及仅能检测但尚未接线执行的内容。

### 当前支持

| 功能 | 映射方式 | 适用范围 |
| ------------- | ------------------------------------------------------------------------------------------- | -------------- |
| Skill 内容 | bundle 的 skill 根目录按普通 OpenClaw skills 加载 | 所有格式 |
| Commands | `commands/` 和 `.cursor/commands/` 被视为 skill 根目录 | Claude、Cursor |
| Hook 包 | OpenClaw 风格的 `HOOK.md` + `handler.ts` 布局 | Codex |
| MCP 工具 | bundle MCP 配置合并到内置 Pi 设置中；加载受支持的 stdio 和 HTTP 服务器 | 所有格式 |
| LSP 服务器 | Claude `.lsp.json` 和 manifest 声明的 `lspServers` 合并到内置 Pi LSP 默认值中 | Claude |
| 设置 | Claude `settings.json` 作为内置 Pi 默认值导入 | Claude |

#### Skill 内容

- bundle 的 skill 根目录按普通 OpenClaw skill 根目录加载
- Claude `commands` 根目录会被视为附加的 skill 根目录
- Cursor `.cursor/commands` 根目录会被视为附加的 skill 根目录

这意味着 Claude markdown 命令文件通过普通 OpenClaw skill 加载器工作。Cursor 命令 markdown 也通过同一路径工作。

#### Hook 包

- 只有当 bundle hook 根目录使用普通 OpenClaw hook-pack
  布局时，bundle hook 根目录才会生效。当前主要是兼容 Codex 的情况：
  - `HOOK.md`
...
- OpenClaw 会将 bundle MCP 配置合并到生效中的内置 Pi 设置里，作为
  `mcpServers`
- OpenClaw 会在内置 Pi 智能体回合中，通过启动 stdio 服务器或连接到 HTTP 服务器，暴露受支持的 bundle MCP 工具
- 项目本地的 Pi 设置仍会在 bundle 默认值之后生效，因此在需要时，工作区设置可以覆盖 bundle MCP 条目
- bundle MCP 工具目录会在注册前进行确定性排序，因此上游 `listTools()` 顺序变化不会导致 prompt-cache 工具块频繁抖动
...
**HTTP** 默认通过 `sse` 连接到一个正在运行的 MCP 服务器；在请求时也可使用 `streamable-http`：
...
OpenClaw 会使用对 provider 安全的名称来注册 bundle MCP 工具，格式为
`serverName__toolName`。例如，一个键名为 `"vigil-harbor"` 且暴露
...
- 空服务器名会回退为 `mcp`
- 冲突的已清洗名称会通过数字后缀消歧
- 最终暴露的工具顺序会按安全名称确定性排序，以保持重复 Pi 回合中的缓存稳定

#### 内置 Pi 设置

- 当 bundle 启用时，Claude `settings.json` 会作为默认的内置 Pi 设置导入
...
- bundle LSP 配置会合并到生效中的内置 Pi LSP 默认值里
- 当前仅支持运行基于 stdio 的 LSP 服务器；不受支持的传输方式仍会显示在 `openclaw plugins inspect <id>` 中

### 已检测但不会执行

这些内容会被识别并显示在诊断信息中，但 OpenClaw 不会执行它们：
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    hook-pack 目录（`HOOK.md` + `handler.ts`）时，与 OpenClaw 的契合度最高。
  ...
  <Accordion title="Claude bundles">
    ...
    - `.lsp.json` 加上 manifest 声明的 `lspServers` 路径会加载到内置 Pi LSP 默认值中
    - `hooks/hooks.json` 仅检测，不执行
    - manifest 中自定义组件路径是附加式的（扩展默认值，而不是替换默认值）
...
- Skills 和 hook-pack 路径必须保持在插件根目录内（有边界检查）
- 设置文件会以相同的边界检查方式读取
- 受支持的 stdio MCP 服务器可以作为子进程启动

这使得 bundles 在默认情况下更安全，但对于它们实际暴露的功能，你仍应将第三方 bundles 视为受信内容。
...
<AccordionGroup>
  ...
  <Accordion title="Claude 设置没有生效">
    仅支持来自 `settings.json` 的内置 Pi 设置。OpenClaw 不会
    将 bundle 设置视为原始配置补丁。
  </Accordion>
